...
foreach ($pnRegistros as $pn => $weeks) {
    ...
    $timeProcess = mysqli_query($con, "SELECT  SUM(`QtyTimes`) as cuentas FROM `routing_models` WHERE `pn_routing`='$pn' and `work_routing`='10081' ORDER BY `id_routing` ASC; ");

    while ($row = mysqli_fetch_assoc($timeProcess)) {

        echo "<tr><td>{$pn} - total Terminales: {$row['cuentas']}</td>";
        $rowTotal = 0;
        ...
        foreach ($allWeeks as $week => $_) {
            ...
            if ($value > 0) {
              // 
              $valoresCorte=[];
              $tiempoEnBase= $row['cuentas'] * 6.03;
                $times = ($tiempoEnBase * $value);
                $perDay = $times / 5;
                $valores=$value/5;
                for($i=0; $i<5; $i++){
                    if($value/(5-$i)>1){
                ...
                for ($i = 0; $i < 5; $i++) {
                    ...
                    if(($valoresCorte[$i]<=0)){
                        ...
                        $perDay=round($tiempoEnBase * $valoresCorte[$i] / 60, 2);
