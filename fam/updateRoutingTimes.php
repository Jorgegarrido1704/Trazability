...
$registrosMPS = mysqli_query($con,"SELECT DISTINCT pn_routing FROM `routing_models` ORDER BY pn_routing ASC");
   ...
   // echo "<table border='1' cellpadding='5' cellspacing='0' align='center' style='width: 60%;'>";
    ...
    foreach ($procesos as $key => $valor) {
      //  echo "<tr><td>{$pn}</td><td>{$key}</td>";
       ...
       $setupt = $assetsProcess[$key]>0?round($assetsProcess[$key]/60,3):0;

        $valor = round($valor*1.15,2);
...
header("Location: famTimesRouting.php");
?>
