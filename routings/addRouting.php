...
if($numeroRuteo!="" and $descripcionRuteo!="" and $possibleAsset!=""){
    $existe=mysqli_query($con,"SELECT * FROM routing_process WHERE routingNumber='$numeroRuteo' AND routingDescription='$descripcionRuteo' AND posibleAssets='$possibleAsset' limit 1");
    $rownum=mysqli_num_rows($existe);
    if($rownum>0){
        echo "<script>alert('El registro ya existe');window.location='addRouting.php';</script>";
    }else{
...
$routings = mysqli_query($con,"SELECT * FROM routing_process  ");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Agregar tipo de Ruteo</title>
    <style>
        body{
...
<body>
    <div class="container align-items-left">
        <button class="btn btn-success"><a href="addProcess.html" id="principal">Agrgar procesos</a></button>
    </div>
    <div class="row">
